class Game extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("Btop", "ground/top.png");
    this.load.image("Bbottom", "ground/bottom.png");
  ...
  addDevilsAndProducts() {
    ...
    this.time.addEvent({
      delay: (4000 * 100) / this.player.speed,
  ...
  generateDevilAndProducts() {
    ...
    let x = this.player.x + 750;
    let y = this.randomBetween(400, 800);
    this.generateBread(x, y);

    if (this.randomBetween(1, 10) < 5) {
      setTimeout(() => {
        let productX = this.player.x + 750;
        let productY = this.randomBetween(300, 900);
  ...
  generateBread(x, y) {
    let scale = 1;
    const breadUp = this.physics.add.sprite(x, y - 525, "Btop").setScale(scale);
    const breadDown = this.physics.add
      .sprite(x, y + 525, "Bbottom")
      .setScale(scale);

    const breadCOllideUp = this.physics.add
      .sprite(x, y - 525, "Btop")
      .setVisible(false)
      .setScale(0.6, scale * 0.93);
    const breadCOllideDown = this.physics.add
      .sprite(x, y + 525, "Bbottom")
      .setVisible(false)
      .setScale(0.6, scale * 0.93);

    this.physics.add.overlap(this.player, breadCOllideUp, () => {
      if (!this.player.lost) {
        this.player.lost = true;
      }
    });
    this.physics.add.overlap(this.player, breadCOllideDown, () => {
      if (!this.player.lost) {
        this.player.lost = true;
      }
    });
    setTimeout(() => {
      breadUp.destroy();
      breadDown.destroy();
      breadCOllideUp.destroy();
      breadCOllideDown.destroy();
    }, 15000);
  }
